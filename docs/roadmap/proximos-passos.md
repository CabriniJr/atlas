...
| 1.4 | ✅ **Rotação da chave mestra** (`secrets_store.rotate_key` + `scripts/rotate_secret_key.py`, backup automático) **+ UX de cadastro** (auto-registro com `ATLAS_SIGNUP_CODE` → "criar conta" no login; papel sempre member). Convite individual por link fica em backlog. | Operação seborosa a longo prazo; hoje só admin cria por API | M |
