...
| E7-28 | **Endurecimento do modo `code`**: workspace restrito, gate de curadoria humana, persistência de runs, allow/deny de tools por Agente | **em curso** — ✅ ADR-0028; ✅ workspace restrito + allow/deny de tools + teto de concorrência + flag de gate (`build_tool_args`/`resolve_workspace`/`active_runs_count`); falta persistência de runs + UI de curadoria | [ADR-0028](../arquitetura/adr/ADR-0028-endurecimento-agente-code.md), [ADR-0025](../arquitetura/adr/ADR-0025-agente-modo-code.md) §Pendências, [api.py](../../src/atlas/api.py) |
