...
| E9-10 | **Pool de execução**: várias traduções em lote ao mesmo tempo — teto de concorrência escalável em runtime ("réplicas"), fila FIFO, novo `fase=fila`, visibilidade agregada (`GET /_traducao_pool`) + painel na UI | a implementar | [ADR-0038](../arquitetura/adr/ADR-0038-pool-execucao-traducao.md) |
