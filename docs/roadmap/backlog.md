...
| E7-33 | **Sistema multiusuário**: a aplicação terá mais de um usuário — modelo de identidade, isolamento/escopo de recursos por usuário, permissões. Impacta auth da API (hoje token único/loopback) e `ATLAS_ALLOWED_USER_ID`. **A desenhar (ADR).** | proposto | [ADR-0005?](../arquitetura/adr/) (auth), [seguranca](../arquitetura/seguranca.md) |
| E7-34 | **SSO no front p/ GitHub e Claude** (baixa prioridade): login/credenciais de GitHub e Claude via SSO na própria UI, em vez de tokens em `.env`. Depende do sistema multiusuário (E7-33). | proposto (baixa prioridade) | [[atlas-credentials]], E7-33 |
...
| E7-40 | **Bug conhecido: schema do Job × spec gravado.** O schema (`api_schema.py`) usa `agenda`/`modelo`, mas `sync.py` e os renders gravam/leem `schedule`/`model`. Jobs criados via ＋Novo (form do schema) divergem dos criados por código/IA. Unificar nomes (preferir `schedule`/`model`) | proposto | [api_schema.py](../../src/atlas/api_schema.py), [sync.py](../../src/atlas/sync.py) |
