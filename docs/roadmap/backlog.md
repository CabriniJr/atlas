...
| E7-45 | **Agente modo `code` via Ollama nativo** (segundo motor 2b, pedido do PO, prioridade máxima): loop de tool-calling próprio (`agente_ollama.py`) contra `POST /api/chat` do Ollama — catálogo `read_file`/`write_file`/`edit_file`/`list_dir`/`run_command` confinado ao workspace (ADR-0028), allow/deny aplicado em processo; `_run_agent_bg` despacha por motor resolvido (claude/ollama) com fallback pro claude se o endpoint estiver fora do ar antes do 1º turno; erro de UMA tool vira evento `warning` (não termina o run); custo sempre 0; dashboard renderiza os eventos sem mudança (mesmo formato do stream-json do claude) | **feito** | [ADR-0042](../arquitetura/adr/ADR-0042-agente-code-via-ollama.md), [agente_ollama.py](../../src/atlas/agente_ollama.py), [api.py](../../src/atlas/api.py) (`_run_agent_bg_ollama`), [kinds/agente.js](../../src/atlas/dashboard/kinds/agente.js) |
| E7-46 | **Qualidade/auto mode do Agente via Ollama** (fecha pendências do ADR-0042): `search_text`/`find_files` (grep/glob reais no catálogo de tools); CLAUDE.md injetado (cacheado por mtime) no system prompt de todo run modo=code; `spec.max_turnos` configurável (default 40); `POST /_self_restart` admin-only — reinício destacado (`start_new_session`) do processo local após commit, sem humano rodando systemctl; aba 🤖 Auto no dashboard mostra motor/modelo real, teto de turnos, gate, workspace e tools; **`/code [agente]` no terminal global** entra em modo dev (prompt vira `🤖<agente>$`, texto livre despacha pro agente, stream inline, fila FIFO client-side, `/exit` sai) — UX o mais próxima possível do Claude Code CLI, pedido explícito do PO | **feito** | [ADR-0044](../arquitetura/adr/ADR-0044-qualidade-e-auto-mode-agente-ollama.md), [agente_ollama.py](../../src/atlas/agente_ollama.py), [api.py](../../src/atlas/api.py) (`_claude_md_context`, `_agendar_self_restart`), [kinds/agente.js](../../src/atlas/dashboard/kinds/agente.js) (`_tabAuto`, `_appendCodeEventToEl`), [main.js](../../src/atlas/dashboard/main.js) (`/code`, `_cliStartRun`, `_cliQueueOrRun`) |
...
| E9-09 | **Sumário (TOC) + hyperlinks + folio no render editorial** | **feito** — links preservados (URI+GOTO+NAMED, 0→4289); TOC dividido em 1 entrada/linha com leader e `target-counter` (página recalculada); folio do rodapé fiel (régua + capítulo + nº recto/verso via `@page :left/:right`); margens do original. **Correção (2026-07-02):** `_geometria` inflava a margem direita/inferior em páginas com só linhas curtas (sumário/lista/código) ou documentos com pouco texto — coluna útil ficava estreita demais e o texto traduzido (mais longo) colidia/paginava em cima do folio. Fix: quantil alto pooled no documento inteiro (não mediana por página) + piso mínimo de largura/altura útil | [ADR-0036](../arquitetura/adr/ADR-0036-render-editorial-modelo-semantico.md) |
...
| E9-02 | **Sub-projeto B** — qualidade AI-augmented: refino em lotes maiores + comparador opt-in (`spec.comparador`) | **feito** — `lote_refino` default 60; `unificar_termos`/`aplicar_unificacao` (mapa de unificação determinístico pré-render) | [ADR-0034](../arquitetura/adr/ADR-0034-refino-lotes-e-comparador.md) |
...
| E9-06 | **Pilar 1 — job pausável/reagendável por escassez** (core Kind-agnóstico): pausa+checkpoint e **retoma sozinho** após a janela de quota (`spec.janela_retomada_seg`, default 5h) | **feito** — `retomada.py` + varredura no loop; tradução parcial pausa e reagenda. **Correção (2026-07-02):** a varredura de retomada rodava no *mesmo* `try/except` do long-poll do Telegram em `app.py` — se `adapter.receber()` falhasse (ex.: token inválido), a exceção pulava `retomar_pausados` **toda vez**, travando a auto-retomada em silêncio. Extraído `ciclo_scheduler()` (agenda+alarmes+retomada) pro seu próprio `try/except`, independente do Telegram | [ADR-0035](../arquitetura/adr/ADR-0035-job-pausavel-reagendavel.md) |
| E9-10 | **Pool de execução**: várias traduções em lote ao mesmo tempo — teto de concorrência escalável em runtime ("réplicas"), fila FIFO, novo `fase=fila`, visibilidade agregada (`GET /_traducao_pool`) + painel na UI | **feito** | [ADR-0038](../arquitetura/adr/ADR-0038-pool-execucao-traducao.md) |
| E9-11 | **Paralelismo de páginas dentro de um job** (mesmo dial de réplicas do E9-10) + **retry curto persistido** (timeout: até `max_tentativas_timeout`× a cada `janela_retry_timeout_seg` antes de cair pra escassez confirmada de 5h) — `status.tentativas_timeout` sobrevive a restart | **feito** — `pipeline.py` (ThreadPoolExecutor por página) + `traducao_ia.py` (classifica timeout vs erro) + `rotinas/traduzir_pdf.py` (decide janela curta/longa) | [ADR-0039](../arquitetura/adr/ADR-0039-paralelismo-paginas-e-retry-timeout.md) |
| E9-12 | **Fallback bidirecional claude↔ollama** em `ia.invocar` (modelo correto por motor; nunca herda o do outro) + **`Traducao.spec.agente_refino`**: Agente/LLMProvider configurável dita motor/modelo/persona do refino, mesmo padrão do `Repo.analyze_agente` | **feito** — `ia.py` (`modelo_padrao`, `_chamar_claude`/`_chamar_ollama`), `traducao_ia.py` (`resolver_agente_refino`, `instrucao_refino`), `rotinas/traduzir_pdf.py` | [ADR-0040](../arquitetura/adr/ADR-0040-fallback-motor-ia-e-agente-refino.md) |
| E9-13 | **Fidelidade tipográfica real + paginação adaptativa** no render editorial: fonte embutida (não mais genérica), ênfase inline (negrito/itálico no meio do bloco), cor de link herdada (não mais azul fixo), listas numeradas + zero bloco descartado sem tradução, papel `nota_rodape_original` distinto do fólio, **fólio dinâmico** via `string-set` (número igual ao original, escala sozinho em página de continuação), **quebra de página por nível de heading extraída** do próprio PDF (clustering + taxa de "abre página" por documento) | proposto | [ADR-0041](../arquitetura/adr/ADR-0041-fidelidade-tipografica-e-paginacao-adaptativa.md) · [spec](../specs/traducao-render-editorial.md) |
| E9-14 | **Recuperação de jobs órfãos no boot** (`Traducao` presa em `traduzindo`/`fila`/`retomando` por um restart anterior volta pra `pausado` com retomada imediata) + **serialização de chamadas Ollama** (`_OLLAMA_SEMAFORO`, teto `ATLAS_OLLAMA_MAX_CONCURRENT=1`) — evita falso-positivo de fallback claude↔ollama por fila do lado de um servidor local que processa sequencialmente | **feito** — `retomada.py` (`recuperar_orfaos_no_boot`), `app.py` (chamada no boot), `ia.py` (`_OLLAMA_SEMAFORO`) | [ADR-0043](../arquitetura/adr/ADR-0043-recuperacao-orfaos-boot-e-serializacao-ollama.md) |
...
| E10-02 | **Fila + concorrência + persistência + integridade**: pool à la ADR-0038 (`ATLAS_TORRENT_MAX_CONCURRENT=3` baixando juntos, resto na fila FIFO, fase `fila`, porta WebUI + profile por-infohash); **persistência** (`retomar_no_boot` re-despacha os que estavam baixando/na fila após restart — o nox recontinua do parcial em disco); **verificação de integridade pós-download por magic header** (`torrent/integridade.py`: `.nsp`/`.nsz`→`PFS0`, `.xci`→`HEAD`, zip/pdf/7z/iso/xz/… — pega fake/corrompido na fonte, "invalid pfs0"; avisa + marca, não apaga) | **feito** — `torrent/{pool,integridade}.py`, `servico.py` (fila/`retomar_no_boot`/`ao_concluir_slot`/integridade), `download.py` (`alocar_porta`/`profile_para`/porta por instância), `app.py`, `dashboard/kinds/torrent.js` | [ADR-0049](../arquitetura/adr/ADR-0049-kind-torrent-e-conversa-stateful-telegram.md) |
| E10-03 | **VPN kill-switch ligado por padrão** quando o PO tiver VPN no desktop (hoje `permitir_sem_vpn=true` por decisão do PO) | proposto | ADR-0049 §Pendências |
| E10-04 | **Antivírus opcional pós-download** (`torrent-scan.py --clam` no conteúdo baixado) | proposto | ADR-0049 §Pendências |
