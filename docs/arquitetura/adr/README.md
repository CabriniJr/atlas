...
| [0047](ADR-0047-fidelidade-nota-rodape-e-regua-capitulo.md) | **Fidelidade de conteúdo/gráfica render-time**: marcador de nota de rodapé (span sobrescrito) re-elevado a sobrescrito Unicode na prosa traduzida ("membro 1 da" → "membro¹ da"); régua do abre-capítulo O'Reilly reproduzida por detecção do traço real no original — ambas sem re-tradução | **aceito** | PO: "o mais importante para mim agora é fidelidade gráfica e de conteúdo, faça mais samples" |
| [0048](ADR-0048-fallback-escalado-e-pausa-cooperativa.md) | **Fallback escalado e visível (Ollama→Claude)** no nível do job (retry rápido por conexão → muta `cfg.motor` → migra o restante, `status.motor_efetivo` + badge) + **pausa cooperativa honrada nos dois loops** (conserta o ⏸ ignorado em paralelo) — *supersede a postura de fallback do ADR-0045*, refina 0040/0039 | **aceito** | PO: "garantir que vai tentar rodar pelo ollama a tradução e fallback para claude se precisar"; controle refinado |
| [0049](ADR-0049-kind-torrent-e-conversa-stateful-telegram.md) | **Kind `Torrent` (download headless) + conversa stateful no Telegram**: recebe o `.torrent` como anexo → verifica (`scan`) → **pergunta** → baixa via `qbittorrent-nox` (sem GUI) → notifica em marcos (10/50/90%) e ao terminar; `progresso`/`cancelar`; paridade CLI. Primeiro fluxo conversacional stateful (estado no recurso) | **aceito** | PO: "mandar o torrent via telegram… bot de conversa… me pergunte se pode instalar… ver o progresso… notificação quando acabar… via cli para não depender do app abrir" |
